...
public class JiuzhentongzhiEntity<T> implements Serializable {
	...
	/**
	 * 发送状态（0：待发送，1：已发送，2：发送失败，3：发送失败（已达到最大重试次数））
	 */
	...
	@JsonFormat(locale="zh", timezone="GMT+8", pattern="yyyy-MM-dd HH:mm:ss")
	@DateTimeFormat 		
	private Date zuihoufasongshijian;
	
	/**
	 * 通知类型（0：预约成功提醒，1：就诊前1天提醒，2：就诊前1小时提醒）
	 */
	 ...
	 * 设置：最后发送时间
	 */
	public void setZuihoufasongshijian(Date zuihoufasongshijian) {
		this.zuihoufasongshijian = zuihoufasongshijian;
	}
	/**
	 * 获取：最后发送时间
	 */
	public Date getZuihoufasongshijian() {
		return zuihoufasongshijian;
	}
	/**
	 * 设置：通知类型（0：预约成功提醒，1：就诊前1天提醒，2：就诊前1小时提醒）
	 */
	public void setTongzhileixing(Integer tongzhileixing) {
		this.tongzhileixing = tongzhileixing;
	}
	/**
	 * 获取：通知类型（0：预约成功提醒，1：就诊前1天提醒，2：就诊前1小时提醒）
	 */
